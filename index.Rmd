...
The application is described in the paper @Locard-Paulet2018. 
It is a web application accessible at the address given by the instructor. 

## The proteasome

Many information on the 20S core particule of the proteasome (constitutive and immuno) can be found here @Gersch2015.

### Useful information

Composition of the core particule of the proteasome:

The 20S core particule of the proteasome is constituted of 4 rings constituted by 7 subunits each in a \( \alpha \ 7 \):\( \beta \ 7 \):\( \beta \ 7 \):\( \alpha \ 7 \) stoichiometry. Amongst these, only \( \beta \ 1 \), \( \beta \ 2 \) and \( \beta \ 5 \) are catalytically active, and during inflammation, they can be exhanged with alternative \( \beta \ 1i \), \( \beta \ 2i \) and \( \beta \ 5i \) to form the immunoproteasome.

Its most common post-translational modificaitons (PTMS):
...
The aim of this exercise is to compare the composition of commercial standard 20S (std20S) and immuno-20S (i20S) from Enzo Life science. 

### Input data

Data after RoWinPro deconvolution:

* Top-down MS analysis of the std20S: `ProteasomeCommercial_std20S_MS.csv`
* Top-down MS analysis of the i20S: `ProteasomeCommercial_i20S_MS.csv`

### Step by step instructions

1. Load files in the `MS` panel of VisioProt-MS. You can (should):
...
2. Zoom in on the region of interest.
...
* Is the std20S pure?
* Is the i20S pure?
* Make a figure to illustrate your findings.

## 2. Home-made purification of i20S using Size-exclusion chromatography (SEC)

\( \beta \ 1i \), \( \beta \ 2i \) and \( \beta \ 5i \) were expressed in HEK293T-EBNA and proteasome were immuno-purified with an anti-\( \alpha \ 2 \). Then, a SEC was performed to separate the std20S and i20S for their separate characterisation.

![SEC of the immuno-enriched proteasome samples](SEC.png)
...
These are the 2 components of the [Heterogeneous nuclear ribonucleoproteins C1/C2](https://www.uniprot.org/uniprot/P07910) that bind to mRNA. 

### Input data

The sample before SEC has been analysed by top-down MS and MS/MS. The associated files are:

* MS trace of the sample after RoWinPro deconvolution: `ProteasomeBeforeSEC_MS.txt`
* MS/MS: `ProteasomeBeforeSEC_MSMSSpectrumInfo.txt`
* Results of the MS/MS search (Biopharma Finder): `ProteasomeBeforeSEC_PSMs.txt`

Analysis of the fractions eluted from SEC in top-down MS. Data after RoWinPro deconvolution:

* Top-down MS analysis of fraction 1: `ProteasomeFraction01_MS.csv`
* Top-down MS analysis of fraction 2: `ProteasomeFraction02_MS.csv`
* Top-down MS analysis of fraction 3: `ProteasomeFraction03_MS.csv`

### Step by step instructions

1. Load files corresponding to the 3 fractions after SEC in the `MS` panel of VisioProt-MS. Like for the previous part, you can (should):
    * load them one at a time to have a feel for their respective signal and adapt the signal threshold for plotting. This will also allow you to check which proteoforms are the most abundant in case of clusters of elution peaks.
    * load them both at the same time to directly identify which proteoforms are present in only one or the two samples.
  
<div class = "orange">
<strong>!</strong> Loading 3 files at a time can be long. It can help to uncheck the `show data label` box and to reduce the threshold (amount of data plotted).
</div>
...
<div class = "blue">
<strong>Tip:</strong> You can try overlaying the top-down MS runs of the commercical std20S and i20S used in the first part of this workshop to identify the subunits of the core particule of the proteasome. Be aware that there may be a shift in retention times though.
</div>

### Questions

* Do you find all the subunits expected in the i20S? Did the SEC allow a the purification of the i20S? If yes, what fraction would you choose to study the i20S?
* Are some subunits modified? @Gersch2015 detected \( \alpha \ 7 \) with a phosphorylation. Can you see it in any of the fractions?
...
1. Check the `MS mode` box `MS/MS` box to access the MS/MS panel of VisioProt-MS.
2. In `Origin of the MS/MS files`, check `Proteome Discoverer`.
3. Load the MSMSspectrum info and PSM files.

<div class = "yellow">
<strong>-></strong>  The data from the MS/MS acquisition appear. 
</div>

4. If you want to load the corresponding MS trace, select the file in `Select input file for MS`. 

<div class = "yellow">
<strong>-></strong>  The corresponding MS trace appears. Since the acquisition parameters are adapted to a MS/MS run, the retention times, detection limits and sensitivity may be different from a regular top-down MS run.
</div>
