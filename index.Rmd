...
And you can find more delta-masses online, for example at [http://www.unimod.org](http://www.unimod.org).
...
# 1. Comparison of commercial samples of the standard and immunoproteasome core particules
...
\( \beta1i \), \( \beta2i \) and \( \beta5i \) were expressed in HEK293T-EBNA and the proteasome core particules were immuno-purified (IP) with an anti-\( \alpha2 \). Then, a SEC was performed to purify the i20S for further characterisation.
...
1. Load the files corresponding to the 3 fractions after SEC in the `MS` panel of VisioProt-MS. Like for the first part of the workshop, you can:
    * load them one at a time to have a feel for their respective signal and adapt the signal threshold for plotting. This will also allow you to check which proteoforms are the most abundant in case of clusters of elution peaks.
    * load them both at the same time to directly identify which proteoforms are present in only one or the two samples.
...
### 3. MS/MS analysis of std20S after IP

A sample of immuno-purified std20S before SEC has been analysed by top-down MS and MS/MS. The associated files are:

* MS trace of the sample after RoWinPro deconvolution: [stdProteasomeBeforeSEC_MS.txt](input_data/stdProteasomeBeforeSEC_MS.txt)
* MS/MS: [stdProteasomeBeforeSEC_MSMSSpectrumInfo.txt](input_data/stdProteasomeBeforeSEC_MSMSSpectrumInfo.txt)
* Results of the MS/MS search (Biopharma Finder): [stdProteasomeBeforeSEC_PSMs.txt](input_data/stdProteasomeBeforeSEC_PSMs.txt)

If you still have time, you can load this set of MS/MS data:
...
You can now play around with these data and check which MS/MS led to the identification of subunits of the std20S proteasome.

<br>

<div class = "orange">
<strong>!</strong>  Since the acquisition parameters are adapted to a MS/MS run, the retention times, detection limits and sensitivity of the MS data may be different from a regular top-down MS run.
</div>
